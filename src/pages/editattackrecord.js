...
useEffect(() => {
    const fetchDetails = async () => {
      const index = parseInt(router.query.index, 10); // 從查詢參數中獲取索引並轉為數字
      if (gameDocIds.length > 0) {
        const gameRef = doc(firestore, 'team', teamId, 'games', timestamp);
        const gameSnap = await getDoc(gameRef);
        if (gameSnap.exists()) {
          const gameData = gameSnap.data();
          if (gameData.ordermain && gameData.ordermain.length > index) {
            const details = gameData.ordermain[index];
            console.log('detail',details); // 打印特定索引的 ordermain 詳情
          } else {
            console.log('No details available for the given index.');
          }
        }
      }
    };
    ...
    fetchDetails();
   }, [router.query.index, teamId, timestamp, gameDocIds.length, firestore]);
